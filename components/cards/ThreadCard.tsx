...
function ThreadCard({
  ...
  return (
    <article className={`flex w-full flex-col rounded-xl  ${isComment ? 'px-0 xs:px-7 bg-dark-1' : 'bg-dark-2 p-7'}`}>
                    ...
                    <div className={`${isComment && 'mb-10'} mt-5 flex flex-col gap-3`}>
...

                        {
                            isComment && comments.length > 0 && (
                                <Link href={`/thread/${id}`}>
                                    <p className="mt-1 text-subtle-medium text-light-1">{comments.length}</p>
                                </Link>
                            )
                        }
